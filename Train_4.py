...
def decode_and_process(path, label, training):
    img = tf.io.read_file(path)
    img = tf.image.decode_png(img, channels=1)   # grayscale
    img = tf.image.convert_image_dtype(img, tf.float32)  # [0,1]

    # Use resize_with_crop_or_pad for all cases to ensure the image is the correct size
    img = tf.image.resize_with_crop_or_pad(img, IMG_PATCH, IMG_PATCH)

    if training and USE_FLIPS:
        img = tf.image.random_flip_left_right(img, seed=SEED)
        img = tf.image.random_flip_up_down(img, seed=SEED)
...
def gen_batch(file_label_list, training):
    ...
    ds = ds.map(lambda p, l: decode_and_process(p, l, training),
                num_parallel_calls=tf.data.AUTOTUNE)
    ds = ds.batch(BATCH_SIZE).cache().prefetch(tf.data.AUTOTUNE)
    return ds

train_ds = gen_batch(train_list, training=True)
val_ds   = gen_batch(val_list, training=False)
test_ds  = gen_batch(test_list, training=False)
...
class TLU(layers.Layer):
    """Truncated Linear Unit: clamp to [-th, +th] (Keras 3-safe via keras.ops)."""
...
# Cosine LR schedule (tf.keras is fine here)
